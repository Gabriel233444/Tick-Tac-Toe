...
def callback(r,c):
    ...
    if player == 'X' and states[r][c] == 0 and stop_game==False:
        b[r][c].configure(text='X', fg='black', bg='white')
        states[r][c] = 'X'
        player = 'O'

    if player == 'O'and states[r][c] == 0 and stop_game==False:
        b[r][c].configure(text='O', fg='white', bg='black')
        states[r][c] = 'O'
        player = 'X'
...
for i in range(3):
    for j in range(3):
        b[i][j] = Button(font=('Verdana', 56), width=3, bg='brown', command = lambda r=i,c=j: callback(r,c))
